...
void BMCommands::BMMovement::Execute(float deltaTime)
{
    ...
    for (const auto& boxes : dae::CollisionBoxManager::GetInstance().GetAllWallColliders())
    {
        ...
        if (startPos.x + (-1.f * distance) <= boxes->GetCollisionRect().x + boxes->GetCollisionRect().w &&
            startPos.x + -1.f * distance >= boxes->GetCollisionRect().x &&
            startPos.y + (0 * distance) <= boxes->GetCollisionRect().y + boxes->GetCollisionRect().h &&
            startPos.y + 0 * distance >= boxes->GetCollisionRect().y)
        {
            return;
        }
        if (startPos.x + (1.f * distance) <= boxes->GetCollisionRect().x + boxes->GetCollisionRect().w &&
